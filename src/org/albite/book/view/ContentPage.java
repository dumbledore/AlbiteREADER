...
public class ContentPage extends Page {
    ...
    public ContentPage(final Booklet booklet, final Vector regions) {
        super(booklet);
        this.regions = new Region[regions.size()];
        regions.copyInto(this.regions);
    }

    protected void draw(Graphics g, ColorScheme cp, AlbiteFont fontPlain, AlbiteFont fontItalic) {
        if (regions != null) {
            for (int i = 0; i < regions.length; i++) {
                regions[i].draw(g, cp, fontPlain, fontItalic);
            }
        }
    }
}